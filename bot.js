/**
 * VistoJá (by agendamentodevisto.bot) — Telegram Scheduling & Messaging Bot + Painel Admin (tempo real)
 ...
 *            já existente do bot. Todas as perguntas de múltipla escolha
 ...
 *  [MERGE H] NOVO — "Nossos Serviços" (Notificações pagas / Reserva
 *            Automática / Lista de Espera Automática) + assistente de rota
 *            (país de origem → país de destino → cidade →
 *            categoria/subcategoria), inspirado no fluxo enviado pelo
 *            cliente, 100% em português e configurável pelo painel
 *            (coleções origin_countries / routes). Também reorganiza os
 *            botões do menu principal (2 colunas) e limpa a listagem do
 *            calendário (sem ID interno do Firestore / sem UID do usuário —
 *            mostra só o que foi agendado).
 *
 * -----------------------------------------------------------------------
 ...
 *                             settings/email no Firestore.
 *   AGENCY_NAME               Nome de marca exibido nos textos (padrão: "VistoJá").
 *   ADMIN_CHAT_ID             ID numérico do Telegram do admin fixo que recebe
 *                             as notificações de handoff e pode responder
 *                             diretamente por reply, sem precisar abrir o
 *                             painel (ver [MERGE C] / [PARTE 3]). Se não
 *                             definida, cai no fallback ADMIN_UIDS[0] — o
 *                             recomendado é defini-la explicitamente para
 *                             não depender da ordem de ADMIN_UIDS.
 *
 * IMPORTANTE ([MERGE H] — pagamento):
 * Os dados de pagamento (Binance Pay / Express / IBAN) estão fixos no código
 * em PAYMENT_METHODS. Ajuste esses valores antes de subir para produção —
 * especialmente o IBAN, que ainda está como placeholder de zeros.
 *
 ...
 * URLs públicas resultantes do upload continuam gravados no Firestore.
 *
...
// [MERGE H] Nome de marca exibido ao cliente (código interno continua livre).
const AGENCY_NAME = process.env.AGENCY_NAME || 'VistoJá';
const AGENCY_DOMAIN = 'agendamentodevisto.bot';
...
app.get('/', (req, res) => {
  res.status(200).json({
    status: 'online',
    message: `🤖 ${AGENCY_NAME} rodando. Painel em /painel. Use /health para checar status.`,
    timestamp: new Date().toISOString(),
  });
});
...
// [MERGE H] Tenta editar a mensagem atual (quando veio de um clique em botão)
// e, se não der (ex.: mensagem antiga demais / veio de um comando de texto),
// manda uma mensagem nova. Usado bastante no assistente de rota.
async function safeEditOrReply(ctx, text, extra) {
  if (ctx.updateType === 'callback_query') {
    try { await safeEditMessageText(ctx, text, extra); return; } catch (_) { /* cai no reply abaixo */ }
  }
...
// Por isso o gatilho de VAGA CONFIRMADA continua sendo sempre MANUAL:
// alguém da equipe checou o site oficial pessoalmente e confirma pelo painel.
// (O que É automático a partir do [MERGE H] é o MONITORAMENTO contratado
// como serviço pago — ver "Nossos Serviços" abaixo — cujo funcionamento
// operacional roda fora deste arquivo, do lado do painel/infra do cliente.)
// ===========================================================================

const STANDARD_MESSAGES = {
  pedidoAutomacaoSiteOficial:
    'Não faço o agendamento nem monitoro o site oficial de forma gratuita/manual — isso faz parte do nosso serviço pago de Notificações, Reserva Automática ou Lista de Espera Automática. Toque em "🗂️ Nossos Serviços" para ver os detalhes e valores.',
  vagaEncontrada: (rota) =>
    `🔔 Vaga disponível para ${rota.origin} → ${rota.destination}! Acesse o site oficial agora e finalize o agendamento antes que esgote.`,
  recusaConsentimento:
    'Sem autorização para uso dos dados, não consigo continuar com o agendamento. Posso te transferir para um atendente humano?',
  naoSubstituiAconselhamento:
    '🛡️ Nossa equipe é especialista nesse processo e acompanha o site oficial de forma contínua e profissional, aumentamos muito suas chances de conseguir a vaga dentro do prazo',
  documentoExpirado:
...
function looksLikeAutomationRequest(text) {
  ...
  return (
    ...
    (t.includes('site') || t.includes('vfs') || t.includes('bls') || t.includes('tls') || t.includes('oficial'))
  );
}
...
// ===========================================================================
// [MERGE H] "Nossos Serviços" — textos em português + dados de pagamento
// ===========================================================================

const PAYMENT_METHODS = {
  binance: { label: '🟡 Binance Pay', value: 'kumbovd@gmail.com' },
  unitel:  { label: '💳 Unitel Money', value: '973476342' },
  iban:    { label: '🏦 IBAN', value: '005500000456180610181' },
};

// Sites monitorados pelo serviço de Notificações. Confirme/ajuste esta lista
// junto ao painel (era "VFS, BLS, TLS, GVC..." no pedido original).
const MONITORED_SITES = ['VFS Global', 'BLS International', 'TLS Contact', 'GVC'];

// [NOVO SERVIÇO] Rótulos e "chaves" (usadas nos callback_data e no Firestore)
// dos três tipos de serviço oferecidos em "Nossos Serviços". Centralizado
// aqui para não haver divergência de texto entre os vários pontos do bot.
const SERVICE_TYPES = {
  notif: 'notif',
  auto: 'auto',
  waitlist: 'waitlist',
...
const SERVICE_LABELS = {
  notif: 'Notificações',
  auto: 'Reserva Automática',
  waitlist: 'Lista de Espera Automática',
};

const SERVICES_INTRO =
`🗂️ *Nossos Serviços — ${AGENCY_NAME}*

Atuamos com foco em Angola 🇦🇴 e Portugal 🇵🇹, mas também atendemos outros países.

Oferecemos três tipos de serviço:

🔔 *Notificações de vaga* — pago, disponível em mais de 100 países
🤖 *Reserva Automática* — disponível apenas para algumas rotas (Reino Unido, Irlanda e Emirados Árabes Unidos)
📄*Lista de Espera Automática* — Seja adicionado à lista de espera do centro de vistos e receba as credenciais de acesso para concluir a marcação você mesmo
...
🔍 Nossos sistemas monitoram o site do centro de vistos a cada poucos segundos, para a cidade e categoria que você escolher, e avisam você por mensagem direta aqui no Telegram assim que uma vaga é detectada.

Sites monitorados: ${MONITORED_SITES.join(', ')}.

⚠️ Conseguir a vaga *não é garantido* — depende da sua rapidez ao preencher os dados no site oficial e da quantidade de vagas liberadas pelo centro (pode variar de 1 a mais de 100, o que afeta bastante suas chances).

📅 *Duração:* o serviço fica ativo por 30 dias a partir da compra. Não é uma assinatura recorrente — depois dos 30 dias, é preciso fazer um novo pedido. O serviço encerra automaticamente ao fim do prazo.

💵 *Reembolso:* reembolso integral se você não receber nenhuma notificação dentro dos 30 dias. Fora essa situação, não há reembolso.

💰 *Valor:* 20 (USD) por 30 dias.`;

const AUTOMATED_TEXT =
`🤖 *Reserva Automática*

Disponível para: 🇬🇧 Reino Unido, 🇮🇪 Irlanda e 🇦🇪 Emirados Árabes Unidos.

O nosso sistema monitora o site do centro de vistos e reserva automaticamente uma vaga assim que ela aparece — você não precisa ficar de olho em notificações, nós tratamos de todo o processo de reserva.

1️⃣ *Taxa do centro de vistos:* dependendo da cidade, o centro cobra uma taxa de marcação. Cobramos esse valor adiantado para pagar o centro diretamente no momento da reserva. Você recebe a fatura do centro junto com a confirmação do seu agendamento, como comprovativo de que o valor foi usado para garantir sua vaga.

2️⃣ *Envio de dados:* depois de acertada a taxa, você recebe um formulário com os dados necessários para concluir a reserva.

3️⃣ *Alocação e confirmação:* começamos a procurar a vaga assim que recebemos o seu formulário preenchido. Existe uma fila — atendemos primeiro quem entrou em contacto antes. O processo pode levar de alguns dias até 4 semanas, dependendo da frequência com que o centro libera vagas. Ao garantir o horário, você é avisado e recebe uma captura de ecrã da reserva; o PDF completo é liberado depois do pagamento do nosso serviço.

💵 *Reembolso:* 80% do valor adiantado caso desista antes de a vaga ser garantida. Depois que a vaga é garantida e a taxa do centro é paga, não é possível reembolsar (os custos já foram efetuados).

📅 *Gestão de datas:* você poderá indicar as datas de preferência para o agendamento após o pagamento. Definir datas muito específicas pode reduzir a chance de encontrar vaga disponível.

🔒 *Privacidade:* seus dados são protegidos conforme o RGPD (UE) e o Data Protection Act 2018 (Reino Unido). Usamos seus dados só para o processo de reserva e apagamos depois. Coletamos apenas o necessário exigido pelos centros de visto. Política de privacidade: @AppointmentsvfsglobalBot`;

// [NOVO SERVIÇO] Texto exibido ao tocar em "📄 Lista de Espera Automática".
const WAITLIST_TEXT =
`📄 Novo serviço disponível: Lista de Espera Automatizada

Esta opção permite-nos adicioná-lo à lista de espera de um centro de vistos, poupando significativamente nos custos do pedido de visto. A disponibilidade e o preço atual são mostrados durante a seleção do serviço e do destino.

Principais vantagens:
✅ Processamento rápido (apenas alguns dias)
✅ Muito mais barato do que uma marcação completa, com a mesma rapidez na obtenção de vaga
✅ Fornecemos os dados de login do e-mail e da conta VFS assim que o serviço for concluído

🔔 Notificações: Receba notificações sobre vagas disponíveis durante o próximo mês, facilitando o agendamento conveniente no centro de vistos. 13 países suportados.`;

function buildPaymentText(order) {
  return (
    `💳 *Pagamento — ${order.serviceLabel}*\n\n` +
    `Valor: *${order.price}*\n\n` +
    `Escolha uma das formas de pagamento abaixo e envie o comprovativo aqui neste chat:\n\n` +
    `${PAYMENT_METHODS.binance.label}: \`${PAYMENT_METHODS.binance.value}\`\n` +
    `${PAYMENT_METHODS.unitel.label}: \`${PAYMENT_METHODS.unitel.value}\`\n` +
    `${PAYMENT_METHODS.iban.label}: \`${PAYMENT_METHODS.iban.value}\`\n\n` +
    `Depois de pagar, envie o print/comprovativo aqui que um atendente confirma o seu pedido, clica aqui para confirmar /atendente.`
  );
}

// [PARTE 1] Helper genérico: adiciona a linha "🔙 Voltar ao Menu" ao final de
// qualquer teclado inline. Usado em todas as telas "terminais" do bot (e,
// por extensão, em todo o assistente de rota "Nossos Serviços") para que o
// usuário nunca fique sem uma forma de voltar ao menu principal sem
// precisar digitar /start de novo.
function withBackButton(rows) {
  return Markup.inlineKeyboard([
    ...rows,
    [Markup.button.callback('🔙 Voltar ao Menu', 'go:menu_principal')],
  ]);
}

// [PARTE 1] Usado nas telas do assistente de rota (wo/wd/wc/wcat/wsub) para
// que uma sessão expirada ou um erro (ex.: rota removida do painel) nunca
// deixe o usuário "preso" na tela anterior sem nenhuma saída — a mensagem é
// substituída por um aviso com o botão de voltar ao menu.
async function safeSessionExpired(ctx, message) {
  try { await ctx.answerCbQuery(message); } catch (_) { /* callback já pode ter expirado */ }
  try {
    await safeEditMessageText(ctx, message, withBackButton([]));
  } catch (_) { /* mensagem pode já não ser editável — sem problema, o toast já avisou */ }
}

// [NOVO SERVIÇO] Teclado padrão com os três serviços, reutilizado em todos os
// pontos do bot onde o menu "Nossos Serviços" é exibido.
// [PARTE 1] Já inclui nativamente o botão "🔙 Voltar ao Menu".
function buildServicesKeyboard() {
  return withBackButton([
    [Markup.button.callback('🔔 Notificações', 'srv:notif')],
    [Markup.button.callback('🤖 Reserva Automática', 'srv:auto')],
    [Markup.button.callback('📄 Lista de Espera Automática', 'srv:waitlist')],
  ]);
}
...
async function startHandoff(ctx, reason) {
  ...
  if (ADMIN_CHAT_ID) {
    // [PARTE 3] A notificação agora pede explicitamente para o admin
    // responder por REPLY a esta mensagem. Guardamos o vínculo
    // message_id -> chatId em "handoff_admin_replies" para o relay de
    // resposta (ver findHandoffChatIdByAdminMessageId e o handler
    // bot.on('text', ...) que escuta respostas do ADMIN_CHAT_ID, mais
    // abaixo neste arquivo).
    const notifMsg = await bot.telegram.sendMessage(ADMIN_CHAT_ID,
      `🧑‍💼 Handoff solicitado\nUsuário: ${user.name || ''}\nchat_id: ${chatId}\nMotivo: ${reason || 'solicitado pelo usuário'}\n\nResponda a ESTA mensagem para falar diretamente com o utilizador.`
    ).catch(() => null);
    if (notifMsg) {
      await db.collection('handoff_admin_replies').doc(String(notifMsg.message_id)).set({
        chatId, createdAt: admin.firestore.FieldValue.serverTimestamp(),
      }).catch((err) => console.error('❌ Erro ao gravar vínculo handoff_admin_replies:', err));
    }
  }
  await sendEmailNotification(
...
async function endHandoff(chatId, resumeMode = 'continue') {
  ...
  if (resumeMode === 'restart') {
    ...
    await safeSend(chatId, 'Retomando o atendimento automático. Clica aqui /start para continuar seu agendamento.');
  }
}

// [PARTE 3] Consulta o vínculo message_id (da notificação enviada ao admin em
// startHandoff) -> chatId (utilizador original). Só retorna o chatId se o
// handoff correspondente ainda estiver ATIVO — evita repassar uma resposta
// tardia do admin para um atendimento que já foi encerrado (seja pelo
// painel, seja por /encerrar_atendimento).
async function findHandoffChatIdByAdminMessageId(messageId) {
  try {
    const snap = await db.collection('handoff_admin_replies').doc(String(messageId)).get();
    if (!snap.exists) return null;
    const { chatId } = snap.data();
    if (!chatId) return null;
    if (!isHandoffActive(chatId)) return null;
    return chatId;
  } catch (err) {
    console.error('❌ Erro ao buscar vínculo handoff_admin_replies:', err);
    return null;
  }
}
...
async function startSlotSelection(ctx) {
  ...
  const buttons = days.slice(0, 14).map(dayKey => {
    ...
    // [PARTE 2] Não exibimos mais a contagem de vagas no texto do botão.
    // `byDay[dayKey].length` continua usado normalmente na lógica interna
    // (ex.: bot.action(/^day:(.+)$/) revalida a quantidade de vagas do dia
    // ao ser clicado) — só o texto visível do botão mudou.
    return [Markup.button.callback(`📅 ${label}`, `day:${dayKey}`)];
  });
...
const guidance = getTravelDocGuidance(data.localOrigem, data.localDestino);
    let msg = `✅ Agendamento criado!\nData: ${st.dt.toLocaleString(DateTime.DATETIME_MED)}\nServiço: ${data.tipoServico}\n\n` +
      `🛫 Agência de Viagens & Vistos — VistoJá

Nossa equipe fornece suporte completo no processo de visto Schengen nacional para diversas categorias.
Também acompanhamos o processo de quem deseja visto de residência ou migração, oferecendo apoio na obtenção dos documentos necessários para agilizar sua solicitação.

🌍 Serviços Principais
Consultoria de Vistos — orientação personalizada para cada tipo de visto.

Apoio a Migrantes — acompanhamento no processo de residência e regularização.

Documentação Rápida — suporte na coleta e preparação dos documentos exigidos.

Notificações Automáticas — alertas imediatos de vagas disponíveis.

Reserva Automática — garantimos sua vaga assim que surgir.

Lista de Espera Automática — inclusão direta nas filas oficiais.

➡️ Nossos bots verificam mais de 10 contas a cada 12 horas nos portais VFS e BLS, garantindo agilidade e eficiência.

Você pode acompanhar o site sozinho — ou deixar essa responsabilidade conosco e ganhar tranquilidade, Clica aqui para voltar ao menu /start falar com /atendente.`;
...
// ===========================================================================
// [MERGE H] Assistente de rota — Nossos Serviços
// País de origem → país de destino → cidade → categoria/subcategoria →
// resumo + instruções de pagamento. Totalmente configurável pelo painel
// através das coleções "origin_countries" e "routes" no Firestore; se essas
// coleções ainda não tiverem sido preenchidas, caímos num fallback com
// Angola/Portugal para não travar o fluxo.
//
// [NOVO SERVIÇO] O mesmo assistente (origem → destino → cidade → categoria
// → subcategoria → resumo/pagamento) agora também atende o serviço
// "waitlist" (Lista de Espera Automática), exatamente como já fazia para
// "notif" e "auto" — só muda o rótulo/preço mostrados no resumo final e,
// opcionalmente, o filtro de rotas elegíveis (campo "waitlist" no doc de
// routes/{id}, igual ao "automated" já usado pela Reserva Automática).
//
// [PARTE 1] Todas as telas deste assistente (origem, destino, cidade,
// categoria, subcategoria) agora trazem o botão "🔙 Voltar ao Menu" — o
// usuário pode desistir do pedido a qualquer momento sem digitar /start.
// ===========================================================================

let originCountriesCache = [];
function watchOriginCountries() {
  db.collection('origin_countries').onSnapshot((snap) => {
    originCountriesCache = snap.docs
      .map(d => ({ id: d.id, ...d.data() }))
      .sort((a, b) => (a.order || 0) - (b.order || 0));
  }, (err) => console.error('❌ Erro no listener de origin_countries:', err));
  console.log('🌍 Países de origem (Nossos Serviços) sincronizados em tempo real com o painel.');
}

// Fallback enquanto o painel não cadastra nada em "origin_countries".
const DEFAULT_PINNED_ORIGINS = [
  { id: 'angola', name: 'Angola', flag: '🇦🇴', pinned: true },
  { id: 'portugal', name: 'Portugal', flag: '🇵🇹', pinned: true },
];

function getPinnedOrigins() {
  const pinned = originCountriesCache.filter(c => c.pinned);
  return pinned.length ? pinned : DEFAULT_PINNED_ORIGINS;
}
function getAllOrigins() {
  return originCountriesCache.length ? originCountriesCache : DEFAULT_PINNED_ORIGINS;
}
function findOrigin(originId) {
  return getAllOrigins().find(c => c.id === originId) || { id: originId, name: originId, flag: '' };
}

async function getRoutesByOrigin(originId) {
  const snap = await db.collection('routes').where('originId', '==', originId).get();
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}

// [NOVO SERVIÇO] Filtra as rotas elegíveis para o serviceType escolhido.
// - "auto"     → exige route.automated === true (igual já era antes)
// - "waitlist" → exige route.waitlist === true (novo campo, configurável
//                pelo painel; se nenhuma rota tiver o campo definido ainda,
//                nada quebra: simplesmente nenhuma rota aparece até o
//                painel marcar as rotas elegíveis)
// - "notif"    → todas as rotas cadastradas para a origem servem
function filterEligibleRoutes(routes, serviceType) {
  if (serviceType === SERVICE_TYPES.auto) return routes.filter(r => r.automated);
  if (serviceType === SERVICE_TYPES.waitlist) return routes.filter(r => r.waitlist);
  return routes;
}

// Estado do assistente de serviços — independente do bookingState (agendar),
// para não conflitar caso o usuário use os dois fluxos.
const wizardState = {};
function clearWizardState(chatId) {
  if (wizardState[chatId] && wizardState[chatId].timer) clearTimeout(wizardState[chatId].timer);
  delete wizardState[chatId];
}
function touchWizardState(chatId, replyFn) {
  const st = wizardState[chatId];
  if (!st) return;
  if (st.timer) clearTimeout(st.timer);
  st.timer = setTimeout(() => {
    clearWizardState(chatId);
    replyFn('⏱️ Pedido cancelado por inatividade. Toque em "🗂️ Nossos Serviços" para começar de novo /start.').catch(() => {});
  }, BOOKING_TIMEOUT_MIN * 60 * 1000);
}

async function startServiceWizard(ctx, serviceType) {
  const chatId = ctx.chat.id;
  clearWizardState(chatId);
  wizardState[chatId] = { step: 'origin', serviceType, data: {} };
  touchWizardState(chatId, (t) => bot.telegram.sendMessage(chatId, t));

  const pinned = getPinnedOrigins();
  const rows = pinned.map(c => [Markup.button.callback(`${c.flag || ''} Aplicar a partir de ${c.name}`.trim(), `wo:${c.id}`)]);
  rows.push([Markup.button.callback('🌍 Aplicar a partir de outro país', 'wo_other:0')]);
  await safeEditOrReply(ctx, 'De onde você está aplicando (país de origem)?', withBackButton(rows));
}

// Tela 1b — lista paginada de todos os países de origem
bot.action(/^wo_other:(\d+)$/, safeAction(async (ctx) => {
  const page = parseInt(ctx.match[1], 10);
  const list = getAllOrigins();
  const pageSize = 8;
  const slice = list.slice(page * pageSize, (page + 1) * pageSize);
  const rows = slice.map(c => [Markup.button.callback(`${c.flag || ''} ${c.name}`.trim(), `wo:${c.id}`)]);
  const nav = [];
  if (page > 0) nav.push(Markup.button.callback('‹ Anterior', `wo_other:${page - 1}`));
  if ((page + 1) * pageSize < list.length) nav.push(Markup.button.callback('Seguinte ›', `wo_other:${page + 1}`));
  if (nav.length) rows.push(nav);
  await ctx.answerCbQuery();
  await safeEditMessageText(ctx, 'Escolha o país de origem:', withBackButton(rows));
}));

// Tela 2 — país de destino (rotas cadastradas a partir da origem escolhida)
bot.action(/^wo:([a-zA-Z0-9_-]+)$/, safeAction(async (ctx) => {
  const chatId = ctx.chat.id;
  const originId = ctx.match[1];
  const st = wizardState[chatId];
  if (!st) return safeSessionExpired(ctx, 'Sessão expirada. Toque em "🗂️ Nossos Serviços" de novo /start.');
  const originInfo = findOrigin(originId);
  st.data.originId = originId;
  st.data.originName = originInfo.name;
  st.step = 'destination';
  touchWizardState(chatId, (t) => bot.telegram.sendMessage(chatId, t));

  const routes = await getRoutesByOrigin(originId);
  await ctx.answerCbQuery();

  const eligible = filterEligibleRoutes(routes, st.serviceType);
  if (!eligible.length) {
    const currentLabel = SERVICE_LABELS[st.serviceType] || 'esse serviço';
    const msg = routes.length
      ? `Ainda não há rotas com ${currentLabel} a partir de ${originInfo.name}. Você pode escolher Notificações para essa origem, ou falar com um atendente.`
      : `Ainda não temos rotas configuradas a partir de ${originInfo.name}. Toque em "🧑‍💼 Falar com atendente" para atendimento manual.`;
    return safeEditMessageText(ctx, msg, withBackButton([
      [Markup.button.callback('🔔 Ver Notificações nessa origem', `wswitch:notif:${originId}`)],
      [Markup.button.callback('🧑‍💼 Falar com atendente', 'go:atendente')],
    ]));
  }

  const rows = eligible.map(r => [Markup.button.callback(`${r.destinationFlag || ''} ${r.destinationName}`.trim(), `wd:${r.id}`)]);
  await safeEditMessageText(ctx, `Origem: ${originInfo.name}\n\nPara onde é o visto (destino)?`, withBackButton(rows));
}));

bot.action(/^wswitch:notif:([a-zA-Z0-9_-]+)$/, safeAction(async (ctx) => {
  const chatId = ctx.chat.id;
  const originId = ctx.match[1];
  const st = wizardState[chatId];
  if (!st) return safeSessionExpired(ctx, 'Sessão expirada.');
  st.serviceType = 'notif';
  await ctx.answerCbQuery();
  const originInfo = findOrigin(originId);
  const routes = await getRoutesByOrigin(originId);
  const rows = routes.map(r => [Markup.button.callback(`${r.destinationFlag || ''} ${r.destinationName}`.trim(), `wd:${r.id}`)]);
  await safeEditMessageText(ctx, `Origem: ${originInfo.name}\n\nPara onde é o visto (destino)?`, withBackButton(rows));
}));

// Tela 4 — cidade (centro de vistos)
bot.action(/^wd:(.+)$/, safeAction(async (ctx) => {
  const chatId = ctx.chat.id;
  const routeId = ctx.match[1];
  const st = wizardState[chatId];
  if (!st) return safeSessionExpired(ctx, 'Sessão expirada.');
  const routeSnap = await db.collection('routes').doc(routeId).get();
  if (!routeSnap.exists) return safeSessionExpired(ctx, 'Rota não encontrada.');
  const route = { id: routeId, ...routeSnap.data() };
  st.data.routeId = routeId;
  st.data.destinationName = route.destinationName;
  st.step = 'city';
  touchWizardState(chatId, (t) => bot.telegram.sendMessage(chatId, t));

  const cities = route.cities || [];
  await ctx.answerCbQuery();
  if (!cities.length) return advanceToCategory(ctx, chatId, route, null);
  const rows = cities.map(c => [Markup.button.callback(`📍 ${c.name}`, `wc:${c.id}`)]);
  await safeEditMessageText(ctx, `Destino: ${route.destinationName}\n\nEscolha o centro de vistos (cidade):`, withBackButton(rows));
}));

bot.action(/^wc:(.+)$/, safeAction(async (ctx) => {
  const chatId = ctx.chat.id;
  const cityId = ctx.match[1];
  const st = wizardState[chatId];
  if (!st) return safeSessionExpired(ctx, 'Sessão expirada.');
  const routeSnap = await db.collection('routes').doc(st.data.routeId).get();
  if (!routeSnap.exists) return safeSessionExpired(ctx, 'Rota não encontrada.');
  const route = { id: st.data.routeId, ...routeSnap.data() };
  const city = (route.cities || []).find(c => c.id === cityId);
  st.data.cityName = city ? city.name : cityId;
  await ctx.answerCbQuery();
  await advanceToCategory(ctx, chatId, route, city);
}));

// Tela 5 — categoria / subcategoria
async function advanceToCategory(ctx, chatId, route, city) {
  const st = wizardState[chatId];
  st.step = 'category';
  touchWizardState(chatId, (t) => bot.telegram.sendMessage(chatId, t));
  const categories = route.categories || [];
  if (!categories.length) return finishWizardToPayment(ctx, chatId, route);
  const rows = categories.map(c => [Markup.button.callback(c.name, `wcat:${c.id}`)]);
  await safeEditOrReply(ctx, 'Categoria do visto:', withBackButton(rows));
}

bot.action(/^wcat:(.+)$/, safeAction(async (ctx) => {
  const chatId = ctx.chat.id;
  ...
  if (!routeSnap.exists) return safeSessionExpired(ctx, 'Rota não encontrada.');
  const route = { id: st.data.routeId, ...routeSnap.data() };
  const category = (route.categories || []).find(c => c.id === catId);
  st.data.categoryId = catId;
  st.data.categoryName = category ? category.name : catId;
  await ctx.answerCbQuery();

  const subs = (category && category.subcategories) || [];
  if (!subs.length) return finishWizardToPayment(ctx, chatId, route);
  st.step = 'subcategory';
  touchWizardState(chatId, (t) => bot.telegram.sendMessage(chatId, t));
  const rows = subs.map(s => [Markup.button.callback(s.name, `wsub:${s.id}`)]);
  await safeEditMessageText(ctx, 'Subcategoria:', withBackButton(rows));
}));

bot.action(/^wsub:(.+)$/, safeAction(async (ctx) => {
  const chatId = ctx.chat.id;
  ...
  if (!routeSnap.exists) return safeSessionExpired(ctx, 'Rota não encontrada.');
  const route = { id: st.data.routeId, ...routeSnap.data() };
  const category = (route.categories || []).find(c => c.id === st.data.categoryId);
  const sub = category && (category.subcategories || []).find(s => s.id === subId);
  st.data.subcategoryName = sub ? sub.name : subId;
  await ctx.answerCbQuery();
  await finishWizardToPayment(ctx, chatId, route);
}));

// [NOVO SERVIÇO] Resolve preço + rótulo de exibição de acordo com o
// serviceType do wizard, lendo os campos correspondentes cadastrados na
// rota (routes/{id}.priceNotification / .priceAutomated / .priceWaitlist).
function resolveServicePricing(serviceType, route) {
  const serviceLabel = SERVICE_LABELS[serviceType] || 'Serviço';
  let price;
  if (serviceType === SERVICE_TYPES.auto) {
    price = route.priceAutomated || 'a combinar — fale com um atendente';
  } else if (serviceType === SERVICE_TYPES.waitlist) {
    price = route.priceWaitlist || 'a combinar — fale com um atendente';
  } else {
    price = route.priceNotification || '20 USD / 30 dias';
  }
  return { price, serviceLabel };
}

// Tela 6 — resumo do pedido + instruções de pagamento, e grava em
// "service_orders" para o painel confirmar depois de ver o comprovativo.
async function finishWizardToPayment(ctx, chatId, route) {
  const st = wizardState[chatId];
  const uid = String(chatId);
  const { price, serviceLabel } = resolveServicePricing(st.serviceType, route);

  const orderRef = await db.collection('service_orders').add({
    userUid: uid,
    serviceType: st.serviceType,
    serviceLabel,
    origin: st.data.originName,
    destination: st.data.destinationName,
    city: st.data.cityName || null,
    category: st.data.categoryName || null,
    subcategory: st.data.subcategoryName || null,
    price,
    status: 'pending_payment',
    createdAt: admin.firestore.FieldValue.serverTimestamp(),
  });
  clearWizardState(chatId);
  await logAction('service_order_created', uid, { orderId: orderRef.id, serviceType: st.serviceType });

  const summary =
    '📋 *Resumo do pedido*\n\n' +
    `Serviço: ${serviceLabel}\n` +
    `Origem: ${st.data.originName}\n` +
    `Destino: ${st.data.destinationName}\n` +
    (st.data.cityName ? `Cidade: ${st.data.cityName}\n` : '') +
    (st.data.categoryName ? `Categoria: ${st.data.categoryName}\n` : '') +
    (st.data.subcategoryName ? `Subcategoria: ${st.data.subcategoryName}\n` : '') +
    `\n${buildPaymentText({ serviceLabel, price })}`;

  // [PARTE 1] Resumo do pedido agora também traz o botão "🔙 Voltar ao Menu".
  await safeEditOrReply(ctx, summary, {
    parse_mode: 'Markdown',
    ...withBackButton([]),
  });
  ...
  if (ADMIN_CHAT_ID) {
    await safeSend(ADMIN_CHAT_ID,
      `🆕 Novo pedido de serviço (${serviceLabel})\nchat_id: ${chatId}\nRota: ${st.data.originName} → ${st.data.destinationName}\nValor: ${price}`);
  }
  await sendEmailNotification(
    `🆕 Novo pedido — ${serviceLabel}`,
    `<p><b>chat_id:</b> ${chatId}</p>
     <p><b>Rota:</b> ${escapeHtml(st.data.originName)} → ${escapeHtml(st.data.destinationName)}</p>
     <p><b>Valor:</b> ${escapeHtml(String(price))}</p>`,
    'notifyOnVacancy'
  );
}
...
// [PARTE 1] Extraído do bloco de "análise rápida" do /start para ser
// reaproveitado também pelo botão universal "🔙 Voltar ao Menu"
// (bot.action('go:menu_principal')), evitando duplicar o texto/teclado do
// menu principal em dois lugares diferentes do código.
async function buildMainMenuAnalysis() {
  const byDay = await getAvailableSlotsByDay();
  ...
  return {
    ...
    keyboard: Markup.inlineKeyboard([
      [
        Markup.button.callback('🗂️ Nossos Serviços', 'go:servicos'),
        Markup.button.callback('📅 Agendar', 'go:agendar'),
      ],
      [
        Markup.button.callback('🧑‍💼 Falar com atendente', 'go:atendente'),
        Markup.button.callback('🗓️ Calendário', 'go:calendario'),
      ],
    ]),
  };
}
...
async function handleWelcome(ctx, showAnalysis) {
  ...
  await ctx.reply(
    `Olá ${ctx.from.first_name}! 👋 Bem-vindo ao ${AGENCY_NAME}.

Criado pela equipe The Appointments-vfsglobal.

Desenvolvemos bots que vasculham constantemente os sites dos centros de vistos a cada poucos segundos e te notificam / fazem a marcação por você assim que houver vagas disponíveis. Oferecemos três serviços diferentes:

🔔 Notificações: Receba avisos sobre vagas disponíveis ao longo do próximo mês, facilitando o agendamento conveniente no centro de vistos.
🤖 Marcação Automática: Permita que garantamos uma vaga no centro de vistos em seu nome.
📄 Lista de Espera Automática: Seja adicionado à lista de espera do centro de vistos e receba as credenciais de acesso para concluir a marcação você mesmo.
...
Prefere falar com um humano? Entre em contacto ➡ /atendente@AppointmentsvfsglobalBot.`
  ...
  if (showAnalysis) {
    ...
    // [MERGE H] Layout em 2 colunas: "Nossos Serviços" + "Agendar" numa linha,
    // "Falar com atendente" + "Calendário" na linha seguinte.
    const { text, keyboard } = await buildMainMenuAnalysis();
...
bot.command('help', (ctx) => ctx.reply(
  ...
  '• servicos — ver Nossos Serviços (Notificações / Reserva Automática / Lista de Espera Automática)\n' +
...
bot.command(['servicos', 'services'], async (ctx) => {
  await ctx.reply(SERVICES_INTRO, {
    parse_mode: 'Markdown',
...
// [PARTE 1] Botão universal "🔙 Voltar ao Menu" — disponível em todas as
// telas "terminais" do bot (Nossos Serviços, resumo de pedido de serviço,
// assistente de rota). Limpa qualquer bookingState/wizardState em
// andamento e volta para a "análise rápida" com o menu principal.
bot.action('go:menu_principal', safeAction(async (ctx) => {
  await ctx.answerCbQuery();
  clearBookingState(ctx.chat.id);
  clearWizardState(ctx.chat.id);
  const { text, keyboard } = await buildMainMenuAnalysis();
  await safeEditOrReply(ctx, text, keyboard);
}));

// [MERGE H] "Nossos Serviços" — explicação de cada serviço + início do assistente
bot.action('go:servicos', safeAction(async (ctx) => {
  await ctx.answerCbQuery();
  await safeEditMessageText(ctx, SERVICES_INTRO, {
    parse_mode: 'Markdown',
    ...buildServicesKeyboard(),
  });
}));

bot.action('srv:notif', safeAction(async (ctx) => {
  await ctx.answerCbQuery();
  await safeEditMessageText(ctx, NOTIFICATIONS_TEXT, {
    parse_mode: 'Markdown',
    ...withBackButton([[Markup.button.callback('▶️ Continuar pedido', 'wiz:start:notif')]]),
  });
}));

bot.action('srv:auto', safeAction(async (ctx) => {
  await ctx.answerCbQuery();
  await safeEditMessageText(ctx, AUTOMATED_TEXT, {
    parse_mode: 'Markdown',
    ...withBackButton([[Markup.button.callback('▶️ Continuar pedido', 'wiz:start:auto')]]),
  });
}));

// [NOVO SERVIÇO] Botão "📄 Lista de Espera Automática" → mostra o texto
// explicativo do serviço com o botão "Continuar" (mesmo padrão de notif/auto).
bot.action('srv:waitlist', safeAction(async (ctx) => {
  await ctx.answerCbQuery();
  await safeEditMessageText(ctx, WAITLIST_TEXT, {
    parse_mode: 'Markdown',
    ...withBackButton([[Markup.button.callback('▶️ Continuar pedido', 'wiz:start:waitlist')]]),
  });
}));

bot.action(/^wiz:start:(notif|auto|waitlist)$/, safeAction(async (ctx) => {
  await ctx.answerCbQuery();
  await startServiceWizard(ctx, ctx.match[1]);
}));
...
// [PARTE 3] Admin (ADMIN_CHAT_ID) responde diretamente pelo Telegram a uma
// notificação de handoff, sem precisar abrir o painel: basta dar reply à
// mensagem de notificação recebida (ver startHandoff, mais acima, que grava
// o vínculo message_id -> chatId em "handoff_admin_replies"). Este handler
// fica ANTES dos demais handlers de texto para nunca ser interceptado pelo
// fluxo automático (agendamento, coleta guiada, gatilhos sem barra etc.).
// O caminho já existente pelo painel (dashboard_command "handoff_send")
// continua funcionando sem qualquer alteração — ver executeDashboardCommand.
bot.on('text', async (ctx, next) => {
  ...
  const originalChatId = await findHandoffChatIdByAdminMessageId(ctx.message.reply_to_message.message_id);
  if (!originalChatId) return next();

  const res = await safeSend(originalChatId, ctx.message.text);
  await db.collection('handoff_messages').add({
    chatId: String(originalChatId), from: 'agent', text: ctx.message.text,
    agent: String(ADMIN_CHAT_ID), createdAt: admin.firestore.FieldValue.serverTimestamp(),
  }).catch(() => {});
  await ctx.reply(res === true ? '✅ Resposta enviada.' : '⚠️ Falha ao enviar a resposta.');
});
...
bot.on('text', async (ctx, next) => {
  ...
  if (text.startsWith('/')) {
    if (st) {
      clearBookingState(chatId);
      await ctx.reply('⚠️ Agendamento em andamento foi cancelado porque você enviou um comando. Digite agendar para começar de novo, se quiser ou clica aqui /start .').catch(() => {});
    }
    return next();
  }
...
// "servicos" + Função B: detecção de pedido de automação no site oficial.
bot.on('text', async (ctx, next) => {
  if (ctx.chat.type !== 'private') return next();
  const chatId = ctx.chat.id;
  if (bookingState[chatId] || wizardState[chatId] || isHandoffActive(chatId)) return next(); // já tratado acima
  ...
  if (['servicos', 'serviços', 'nossos servicos', 'nossos serviços'].includes(norm)) {
    return ctx.reply(SERVICES_INTRO, {
      parse_mode: 'Markdown',
      ...buildServicesKeyboard(),
    });
  }
...
bot.command('myappointments', async (ctx) => {
  try {
    ...
    const lines = snaps.docs.map(d => {
      ...
      // [MERGE H] Sem ID interno do Firestore — só o que interessa ao usuário.
...
bot.on('photo', async (ctx) => {
  ...
  try {
    ...
    await ctx.reply(` (${result.storage === 'supabase' ? '' : 'link direto'}).`);
  } catch (err) {
...
// [MERGE H] Listagem do dia LIMPA: nada de ID interno do Firestore nem de
// UID do usuário — mostra só o que foi agendado (horário + serviço, e o tipo
// de documento quando disponível, já que veio da coleta guiada).
bot.action(/^cal_day:(.+)$/, safeAction(async (ctx) => {
  ...
  const lines = snap.docs
    .sort((a, b) => a.data().datetime.toDate() - b.data().datetime.toDate())
    .map(d => {
      const a = d.data();
      const t = DateTime.fromJSDate(a.datetime.toDate()).setZone(TIMEZONE).toFormat('HH:mm');
      const tipoDocumento = a.extended && a.extended.tipoDocumento ? ` (${a.extended.tipoDocumento})` : '';
      return `${t} — ${a.category || 'Serviço'}${tipoDocumento}`;
    });
...
bot.command('encerrar_atendimento', async (ctx) => {
  ...
  await ctx.reply('✅ Atendimento encerrado, fluxo automático retomado para o usuário clica aqui start.');
});
...
async function executeDashboardCommand(type, payload) {
  switch (type) {
    ...
    // [MERGE H] Painel confirma pagamento de um pedido de serviço.
    case 'service_order_confirm': {
      if (!payload.orderId) throw new Error('Campo "orderId" é obrigatório.');
      const ref = db.collection('service_orders').doc(payload.orderId);
      const snap = await ref.get();
      if (!snap.exists) throw new Error('Pedido não encontrado.');
      const order = snap.data();
      await ref.update({ status: 'confirmed', confirmedAt: admin.firestore.FieldValue.serverTimestamp() });
      const label = order.serviceLabel || SERVICE_LABELS[order.serviceType] || 'seu serviço';
      await safeSend(order.userUid, `✅ Pagamento confirmado! Seu serviço de ${label} para ${order.origin} → ${order.destination} está ativo.`);
      await logAction('service_order_confirmed', 'dashboard', { orderId: payload.orderId });
      return;
    }
    default:
...
(async () => {
  ...
  bot.launch();
  console.log(`🤖 Bot Telegram iniciado (polling) — ${AGENCY_NAME}.`);
  console.log('✅ Pronto — servidor HTTP + painel + bot + Supabase + email rodando.');
})();
