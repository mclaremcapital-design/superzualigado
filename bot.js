...
const PAYMENT_METHODS = {
  ...
  express: { label: '💳 Express', value: '945561341' },
  iban:    { label: '🏦 IBAN', value: '000000000000000000000' },
};
...
function buildPaymentText(order) {
  return (
    ...
    `${PAYMENT_METHODS.express.label}: \`${PAYMENT_METHODS.express.value}\`\n` +
...
async function finalizeBooking(ctx, chatId) {
  ...
  try {
    ...
    let msg = `✅ Agendamento criado!\nData: ${st.dt.toLocaleString(DateTime.DATETIME_MED)}\nServiço: ${data.tipoServico}\n\n` +
      'O agendamento em si precisa ser feito por você diretamente no site oficial, assim que houver vaga — Caso tenhas dificuldade ou nao encontra vaga,check conta com nossa equipe. Se você contratar nosso serviço de Notificações, Reserva/Marcação Automática e  Lista de Espera Automática /agendar@AppointmentsvfsglobalBot avisamos você quando surgir uma vaga compatível com tuas informações.';
...
function touchWizardState(chatId, replyFn) {
  ...
  st.timer = setTimeout(() => {
    clearWizardState(chatId);
    replyFn('⏱️ Pedido cancelado por inatividade. Toque em "🗂️ Nossos Serviços" para começar de novo.').catch(() => {});
  }, BOOKING_TIMEOUT_MIN * 60 * 1000);
}
...
bot.action(/^wo:([a-zA-Z0-9_-]+)$/, safeAction(async (ctx) => {
  ...
  const st = wizardState[chatId];
  if (!st) return safeSessionExpired(ctx, 'Sessão expirada. Toque em "🗂️ Nossos Serviços" de novo.');
