...
function watchKeywords() {
  db.collection('keywords').onSnapshot((snap) => {
    ...
    console.log(`🔎 [DEBUG] Cache de keywords atualizado: ${keywordsCache.length} palavra(s)`);
    console.log(JSON.stringify(keywordsCache, null, 2));
...
async function handleGroupKeywordScan(ctx) {
  ...
  console.log(`📩 [DEBUG] Mensagem de grupo: "${text}" — testando contra ${keywordsCache.length} palavra(s)`);
  if (!text) { console.log('⚠️ [DEBUG] Sem texto/caption, ignorando.'); return; }
  for (const keyword of keywordsCache) {
    const matched = matchKeyword(text, keyword);
    console.log(`   🔍 [DEBUG] "${keyword.pattern}" (type=${keyword.type}, active=${keyword.active}) → ${matched ? '✅ MATCH' : '❌ não bateu'}`);
    if (matched) {
      console.log('🔔 [DEBUG] Match encontrado! Gravando em keyword_alerts...');
      const alert = {
...
console.log(`🚪 [DEBUG] Roteador de grupo acionado — chat: "${ctx.chat.title}" (${ctx.chat.id})`);
