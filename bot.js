 ...
 *            Automática) + assistente de rota (país de origem → país de
 *            destino → cidade → categoria/subcategoria), inspirado no
 *            fluxo enviado pelo cliente, 100% em português e configurável
 *            pelo painel (coleções origin_countries / routes). Também
 *            reorganiza os botões do menu principal (2 colunas) e limpa a
 *            listagem do calendário (sem ID interno do Firestore / sem UID
 *            do usuário — mostra só o que foi agendado).
 *
 * -----------------------------------------------------------------------
...
const STANDARD_MESSAGES = {
  pedidoAutomacaoSiteOficial:
    'Não faço o agendamento nem monitoro o site oficial de forma gratuita/manual — isso faz parte do nosso serviço pago de Notificações ou Reserva Automática. Toque em "🗂️ Nossos Serviços" para ver os detalhes e valores.',
  vagaEncontrada: (rota) =>
...
Oferecemos dois tipos de serviço:
...
bot.action(/^wo:([a-zA-Z0-9_-]+)$/, safeAction(async (ctx) => {
  ...
  const eligible = routes.filter(r => st.serviceType !== 'auto' || r.automated);
  if (!eligible.length) {
    const msg = routes.length
      ? `Ainda não há rotas com Reserva Automática a partir de ${originInfo.name}. Você pode escolher Notificações para essa origem, ou falar com um atendente.`
...
async function finishWizardToPayment(ctx, chatId, route) {
  const st = wizardState[chatId];
  const uid = String(chatId);
  const price = st.serviceType === 'auto'
    ? (route.priceAutomated || 'a combinar — fale com um atendente')
    : (route.priceNotification || '20 USD / 30 dias');
  const serviceLabel = st.serviceType === 'auto' ? 'Reserva Automática' : 'Notificações';
...
bot.command('help', (ctx) => ctx.reply(
  ...
  '• servicos — ver Nossos Serviços (Notificações / Reserva Automática)\n' +
...
bot.action(/^wiz:start:(notif|auto)$/, safeAction(async (ctx) => {
  await ctx.answerCbQuery();
...
async function executeDashboardCommand(type, payload) {
  switch (type) {
    ...
    case 'service_order_confirm': {
      ...
      await safeSend(order.userUid, `✅ Pagamento confirmado! Seu serviço de ${order.serviceType === 'auto' ? 'Reserva Automática' : 'Notificações'} para ${order.origin} → ${order.destination} está ativo.`);
