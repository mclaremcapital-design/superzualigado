...
🔒 *Privacidade:* seus dados são protegidos conforme o RGPD (UE) e o Data Protection Act 2018 (Reino Unido). Usamos seus dados só para o processo de reserva e apagamos depois. Coletamos apenas o necessário exigido pelos centros de visto. Política de privacidade: https://${AGENCY_DOMAIN}/privacy-policy`;
...
async function finalizeBooking(ctx, chatId) {
  ...
  try {
    ...
    let msg = `✅ Agendamento criado!\nData: ${st.dt.toLocaleString(DateTime.DATETIME_MED)}\nServiço: ${data.tipoServico}\n\n` +
      'O agendamento em si precisa ser feito por você diretamente no site oficial, assim que houver vaga — não faço isso automaticamente. Se você contratar nosso serviço de Notificações ou Reserva Automática (veja "🗂️ Nossos Serviços"), avisamos você quando surgir uma vaga compatível com sua rota.';
