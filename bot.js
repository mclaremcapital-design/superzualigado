...
async function startServiceWizard(ctx, serviceType) {
  ...
  await safeEditOrReply(ctx, 'De onde você está aplicando (país de origem)?', Markup.inlineKeyboard(rows));
}
...
bot.action(/^wo_other:(\d+)$/, safeAction(async (ctx) => {
  ...
  await safeEditMessageText(ctx, 'Escolha o país de origem:', Markup.inlineKeyboard(rows));
}));
...
bot.action(/^wo:([a-zA-Z0-9_-]+)$/, safeAction(async (ctx) => {
  ...
  const st = wizardState[chatId];
  if (!st) return ctx.answerCbQuery('Sessão expirada. Toque em "🗂️ Nossos Serviços" de novo.');
  ...
  if (!eligible.length) {
    ...
    return safeEditMessageText(ctx, msg, Markup.inlineKeyboard([
  ...
  await safeEditMessageText(ctx, `Origem: ${originInfo.name}\n\nPara onde é o visto (destino)?`, Markup.inlineKeyboard(rows));
}));
...
bot.action(/^wd:(.+)$/, safeAction(async (ctx) => {
  ...
  if (!st) return ctx.answerCbQuery('Sessão expirada.');
  const routeSnap = await db.collection('routes').doc(routeId).get();
  if (!routeSnap.exists) return ctx.answerCbQuery('Rota não encontrada.');
  const route = { id: routeId, ...routeSnap.data() };
  ...
  await safeEditMessageText(ctx, `Destino: ${route.destinationName}\n\nEscolha o centro de vistos (cidade):`, Markup.inlineKeyboard(rows));
}));
...
async function advanceToCategory(ctx, chatId, route, city) {
  ...
  await safeEditOrReply(ctx, 'Categoria do visto:', Markup.inlineKeyboard(rows));
}
...
bot.action(/^wcat:(.+)$/, safeAction(async (ctx) => {
  ...
  await safeEditMessageText(ctx, 'Subcategoria:', Markup.inlineKeyboard(rows));
}));
...
async function finishWizardToPayment(ctx, chatId, route) {
  ...
  await safeEditOrReply(ctx, summary, { parse_mode: 'Markdown' });
...
bot.action('srv:notif', safeAction(async (ctx) => {
  ...
  await safeEditMessageText(ctx, NOTIFICATIONS_TEXT, {
    parse_mode: 'Markdown',
    ...Markup.inlineKeyboard([[Markup.button.callback('▶️ Continuar pedido', 'wiz:start:notif')]]),
  });
}));
...
bot.action('srv:auto', safeAction(async (ctx) => {
  ...
  await safeEditMessageText(ctx, AUTOMATED_TEXT, {
    parse_mode: 'Markdown',
    ...Markup.inlineKeyboard([[Markup.button.callback('▶️ Continuar pedido', 'wiz:start:auto')]]),
  });
}));
...
bot.action('srv:waitlist', safeAction(async (ctx) => {
  ...
  await safeEditMessageText(ctx, WAITLIST_TEXT, {
    parse_mode: 'Markdown',
    ...Markup.inlineKeyboard([[Markup.button.callback('▶️ Continuar pedido', 'wiz:start:waitlist')]]),
  });
}));
